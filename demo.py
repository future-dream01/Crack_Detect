...
parser.add_argument('--model', type=str, default='weights/Mar24-13_16000.pth',
                    help='model path')
parser.add_argument('--img_path', type=str, default='/media/jie/Work/Public_Datasets/pano/20210117/val/JPEGImages',
                    help='image path')
            ...
            for name in files:
                ...
                if len(dts):
                    ...
                    for bb in detections:
                        cv2.putText(np_img, classes[int(bb[-1])] + ': %.2f' % (bb[-2]), (int(bb[0]), int(bb[1])),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (255, 255, 255), 2, cv2.LINE_AA)
                        cv2.rectangle(np_img, (int(bb[0]), int(bb[1])), (int(bb[2]), int(bb[3])), color=(255, 0, 0),
                                      thickness=1,
                                      lineType=cv2.LINE_4)
