...
from utils_scrpits.iou_mask import iou_rle, bboxes_iou
...
class YOLOBranch(nn.Module):
    ...
    def forward(self, raw, img_size, labels=None):
        ...
        nCH = self.num_classes  # number of channels
        ...
        tx_all, ty_all = labels[:, :, 0] * raw.shape[3], labels[:, :, 1] * raw.shape[2]  # 0-nG
        ...
        #print ("1是：",xy_offset[obj_mask])
        #print ("2是：",target[..., 0:2][obj_mask])
        ...
        loss_obj = self.loss4obj(conf[obj_mask], target[..., 4][obj_mask])
        loss_noobj = self.loss4obj(conf[noobj_mask], target[..., 4][noobj_mask])

        loss = loss_xy + 0.5 * loss_wh + loss_obj + loss_noobj
        ngt = valid_gt_num + 1e-16
        self.gt_num = valid_gt_num
        self.loss_str = 'total {} objects: '.format(int(ngt)) \
                        + 'xy/gt {}, wh/gt {} '.format('%.3f'%(loss_xy/ngt), '%.3f'%(loss_wh/ngt))\
                        +'conf {}'.format('%.3f'%loss_obj)
