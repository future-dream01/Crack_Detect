...
from utils_scrpits.iou_mask import iou_rle, bbox_iou
import copy
import cv2
import numpy as np
import tqdm
...
def rect_to_square(img, labels, target_size, pro_type, pad_value=0, aug=False):
    ...
    if labels is not None:
        ...
        labels[:, 0:4:2] *= float(unpad_w/ori_w)
        labels[:, 1:4:2] *= float(unpad_h/ori_h)
        labels[:, 0] += left
        labels[:, 1] += top
...
def ap_per_class(tp, conf, pred_cls, target_cls, rec_thres, iou, pre, rec):
    """ Compute the average precision, given the recall and precision curves.
    ...
    for c in tqdm.tqdm(unique_classes, desc="Computing AP"):
        ...
        if n_p == 0 and n_gt == 0:
            ...
            pre.append(precision_curve[3:])
            rec.append(recall_curve[3:])


    return p, r, ap, unique_classes.astype("int32"), pre, rec